...
- **`applicants`** - portal accounts, `contactOrder` (primary/secondary/tertiary) is
  a _label only_, never an access-control tier - all contacts of one organisation
  have strictly equal permissions
- **`account_requests`** - public registration flow (not yet wired to any endpoint -
  M13/Sprint 12). Partial unique index: one _pending_ request per `contactEmail`
...
distinct from `formal_request_documents`, which holds the applicant's _filled-in_
