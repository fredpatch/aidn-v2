...
- **Applicant** (`applicants` table): email + password only, no OTP. Real applicant
  _account creation_ (self-registration, anti-bot, organisation dedup) is M13/Sprint 12
...
`PORTAL_ORIGIN`) to check the _matching_ cookie first, so a stale staff cookie can't
