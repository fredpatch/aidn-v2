...
export function usePaymentActions(
  ...
  const invoiceMutation = useMutation({
    ...
    onError: (err) => setActionError(apiErrorMessage(err, 'Impossible de mettre en ligne la facture.')),
  });
  ...
  const proofMutation = useMutation({
    ...
    onSuccess: invalidate,
    onError: (err) => setActionError(apiErrorMessage(err, 'Impossible de soumettre la preuve de paiement.')),
  });
  ...
  const validateMutation = useMutation({
    ...
    onSuccess: invalidate,
    onError: (err) =>
      setActionError(apiErrorMessage(err, 'Impossible de valider le paiement.')),
  });
