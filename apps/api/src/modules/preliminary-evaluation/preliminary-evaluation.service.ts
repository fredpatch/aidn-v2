...
import { preliminaryEvaluationForms, phases, requests, documentTemplates, meetings } from "../../shared/db/schema.js";
...
async function toView(
  ...
  if (row.templateId) {
    const [template] = await db.select().from(documentTemplates).where(eq(documentTemplates.id, row.templateId));
...
export async function getForPhase(phaseId: number): Promise<PreliminaryEvaluationView | null> {
  const [row] = await db.select().from(preliminaryEvaluationForms).where(eq(preliminaryEvaluationForms.phaseId, phaseId));
...
export async function makeAvailable(
  ...
  if (existing) {
    ...
    [row] = await db
      ...
      .values({ phaseId, templateId: template.id, madeAvailableAt: new Date(), returnDeadline: deadline })
      .returning();
  }
...
export async function submit(
  ...
  await logAudit({ action: "PRELIMINARY_EVALUATION_SUBMITTED", module: "M3", entityId: row.id, details: { mimeType } });

  return toView(updated);
}

/** Helper for controllers doing the ownership check: phase -> request -> applicantId. */
export async function getRequestIdForPhase(phaseId: number): Promise<{ requestId: number; applicantId: number } | null> {
  const [phase] = await db.select().from(phases).where(eq(phases.id, phaseId));
...
export async function getBundleForRequest(requestId: number): Promise<PreliminaryPhaseBundle> {
  ...
  const [meetingRow] = await db
    .select()
    .from(meetings)
    .where(and(eq(meetings.phaseId, phase.id), eq(meetings.status, "scheduled")))
